// https://gist.github.com/hackprime/717b4b09a7cce7488e82
const TRIPLET_NAMES = [
    null,
    ['тысяча', 'тысячи', 'тысяч'],
  ...
  UNIT_NAMES = [ZERO_NAME, 'один', 'два', 'три', 'четыре', 'пять', 'шесть', 'семь', 'восемь', 'девять'],
  TEN_UNIT_NAMES = [
    'десять',
    'одиннадцать',
    'двенадцать',
...
function pluralEnding(number: number, variants: string[]) {
  var one = variants[0],
    two = variants[1],
    five = variants[2];
...
function numberInWords(number: any) {
  let numberInWords = [],
  ...
  for (i = 0; i < length; i += 1) {
    ...
    digitPosition = pos % 3;
    let digitValue = parseInt(number[i]);
